...
pub unsafe trait ZeroOut: ContiguousBytes {
    fn zero(&mut self) {
        unsafe {
            sodium_memzero(
                self.as_mut_bytes().as_mut_ptr() as *mut _,
                self.as_bytes().len(),
            )
        }
    }
    ...
    unsafe fn copy_and_zero(&mut self, other: &mut Self) {
        assert!(
            ...
            "other must be larger than self"
        );

        assert!(
            (self.as_u8_ptr() < other.as_u8_ptr()
                && self.as_u8_ptr().add(self.size()) <= other.as_u8_ptr())
                || (other.as_u8_ptr() < self.as_u8_ptr()
                    && other.as_u8_ptr().add(other.size()) <= self.as_u8_ptr()),
